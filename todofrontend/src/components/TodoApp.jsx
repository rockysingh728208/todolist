...
const TodoApp = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 p-4 sm:p-6">
      <div className="bg-white shadow-2xl rounded-2xl p-6 sm:p-8 w-full max-w-md sm:max-w-lg md:max-w-xl">
        ...
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row gap-2">
          <input
            ...
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            className="flex-grow border-2 border-gray-300 rounded-xl p-2 focus:outline-none focus:border-purple-500"
          />
          <button
            onClick={createTodo}
            className="bg-gradient-to-r from-green-400 to-green-600 text-white px-5 py-2 rounded-xl hover:opacity-90 transition"
          >
            ➕ Add
          </button>
        </div>
        ...
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row gap-2">
          <input
            ...
            value={id}
            onChange={(e) => setId(e.target.value)}
            className="flex-grow border-2 border-gray-300 rounded-xl p-2 focus:outline-none focus:border-blue-500"
          />
          <button
            onClick={getTodo}
            className="bg-gradient-to-r from-blue-400 to-blue-600 text-white px-5 py-2 rounded-xl hover:opacity-90 transition"
          >
            🔍 Find
          </button>
        </div>
        ...
        {/* Single Todo */}
        {singleTodo && (
          <div className="p-4 mb-4 sm:mb-6 bg-yellow-100 border-l-4 border-yellow-500 rounded shadow">
            <h2 className="text-lg sm:text-xl font-semibold">📌 Todo Details</h2>
            <p className="text-sm sm:text-base">
              <strong>ID:</strong> {singleTodo._id}
            </p>
            <p className="text-sm sm:text-base">
              <strong>Title:</strong> {singleTodo.title}
            </p>
          </div>
        )}

        {/* All Todos */}
        <h2 className="text-lg sm:text-xl font-bold text-gray-700 mb-3">📋 All Todos</h2>
        <ul className="space-y-3 max-h-96 overflow-y-auto">
          {todos.map((t) => (
            <li
              key={t._id}
              className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-100 p-3 rounded-lg shadow hover:shadow-md transition gap-2 sm:gap-0"
            >
              <span className="text-gray-800">{t.title}</span>
              <button
                onClick={() => deleteTodo(t._id)}
                className="bg-gradient-to-r from-red-400 to-red-600 text-white px-3 py-1 rounded hover:opacity-90 transition mt-2 sm:mt-0"
              >
                ❌ Delete
              </button>
            </li>
